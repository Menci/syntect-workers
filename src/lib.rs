    ...
    match ThemeSet::load_from_reader(&mut std::io::Cursor::new(theme_data)) {
        Ok(theme) => {
            ...
            unsafe {
                ...
                result = css_for_theme_with_class_style(&theme, ClassStyle::SpacedPrefixed { prefix: &*prefix });
                Box::from_raw(prefix);
            }

            return GetCssResult { css: result, error: "".to_string() };
        },
        Err(error) => {
            return GetCssResult { css: "".to_string(), error: error.to_string() };
        }
    }
}
...
pub fn highlight(code: String, language: String, prefix: String) -> HighlightResult {
    ...
    if !is_plain {
        match syntax_set.find_syntax_by_extension(&language).or(syntax_set.find_syntax_by_name(&language)) {
            Some(s) => syntax = s,
            _ => ()
        }
    }
    ...
    unsafe {
        ...
        let mut generator = ClassedHTMLGenerator::new_with_class_style(syntax, &syntax_set, ClassStyle::SpacedPrefixed { prefix: &*prefix_str });
